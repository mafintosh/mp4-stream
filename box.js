...
var encodings = require('./box-encodings')
...
Box._encode = function (obj, buffer, offset) {
  ...
  } else if (encodings[type]) {
    var encode = encodings[type].encode
...
Box.decodeWithoutHeaders = function (headers, buffer, start, end) {
  ...
  if (containers[type]) {
    ...
    var decode = encodings[type].decode
...
Box.encodingLength = function (obj) {
  ...
  if (containers[type]) {
    ...
    len += encodings[type].encodingLength(obj)
  } else if (obj.buffer) {
